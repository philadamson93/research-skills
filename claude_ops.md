...
- **`/review-implementation <plan-path>`** — implementation audit of uncommitted code against the plan. Run after non-trivial implementation completes, before `/commit-review`. For VM-bound work it hands off to `/vm-handoff` first, so the handoff doc lands in the same commit-review.
- **`/review-tests <plan-path>`** — test-coverage audit of uncommitted code against the plan. Run after `/review-implementation` when the change introduces new branches / contracts / edge cases worth regression-protection. For VM-bound work it hands off to `/vm-handoff` first, so the handoff doc lands in the same commit-review.
