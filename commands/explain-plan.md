...
## Writing voice — plain English, not AI slop

Everything this skill puts in front of a human — the questions it asks, the HTML prose (TL;DR, "what changed" notes, `.resp` callouts, drill answers), and any edit it applies back into a plan doc — should read like a sharp colleague talking, not like a chatbot. The reader is a busy person reviewing a plan; write the way you'd explain it to them across a desk.
...
- Name the actual object — the file, the table, the step — instead of an abstract noun ("the relevant artifact", "this capability").
- For questions: ask what you'd ask out loud. State the real trade-off in words a non-Claude reader gets on the first pass. Make the option label the plain choice ("Make boxes clickable"), not the mechanism ("Enable node-level drill affordance").
...
2. **TL;DR** — 2–4 sentence framing distilled from the plan's "What you're building" / status preamble. Plain English (see *Writing voice* above) — no jargon, no preamble.
...
Write in plain English — short concrete sentences, plain verbs, no inflated diction (skip "leverage / utilize / robust / seamless / comprehensive"), no filler affirmations. Explain it the way you'd tell a colleague at the whiteboard.
