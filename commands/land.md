---
name: land
description: Merge a finished feature branch into main — the step that closes out a branch, the way /next opens a session and /wrapup closes one. Runs a single-repo sibling-branch/worktree conflict scan (the merge-time coordination check that otherwise falls through the gaps), checks whether the branch was reviewed, whether another branch already replaced it, and whether its test results still describe what will actually land now that main has moved, then — behind an explicit gate — performs the merge (ff-only or PR) and prunes the branch, its worktree, and its board entry, updating the program brief's stage row as it goes. TRIGGER when a feature branch is ready to land: the user says "land this", "merge to main", "ship it", "this is done — merge it", or invokes /land. SKIP for doc-only fixes / minor bug fixes committed directly on main (no branch to land), and when the branch isn't finished (route to /commit-review to land the branch first). Do NOT auto-trigger the merge — the write-to-main always waits behind an AskUserQuestion gate.
---

# land

The missing piece. `/next` opens a session and surveys worktrees; `/wrapup` closes a *session* and preserves its state (committing only at a milestone, via its opt-in commit gate). Neither closes a *branch* — the moment a feature is done and you integrate it into `main`. That moment is where sibling-branch coordination matters most (whichever of two branches touching the same files lands second eats a rebase) and where, historically, nobody runs the check because the merge-to-main *action* had no skill to attach it to. This is that skill.

**Open with a one-line context header naming the repo and the branch being landed** — e.g. `vista-eval · feat/survival-eval-v1_6-runner → main` (add the worktree path when landing from a non-primary checkout). Phil launches sessions across ~50 repos and multiple worktrees of the same repo; stating *which branch is about to hit main* up front makes the whole operation unambiguous. Derive it from `git worktree list` + `git branch --show-current`, and repeat it in the Phase 5 summary so the landed state is self-documenting.
...
- **Skip** for doc-only or minor-bug-fix commits that (per `claude_ops.md` → Branches) go **directly on main** — there's no branch to land.
...
- **Do the branch's test results still describe what will land?** The branch's tests/verification prove the tree that was *checked*; that equals the tree landing on main only when main hasn't advanced past the branch's merge-base. Read it off the **same left-right count** above — the trigger is identical to "Behind main?", but the concern is semantic (do the passing tests still describe the merged result?), not textual (does the ff apply?):
  - **main-ahead == 0** → a clean ff, so the branch's existing test run already covers the exact tree that becomes main. Say so and re-verify nothing — this is the trivial path, keep it unblocked.
  - **main-ahead > 0** → landing needs a rebase / merge-main-in first, yielding an integrated tree that was never actually checked (classic logical merge: branch green alone, main green alone, merged tree possibly broken). Run the **Phase 2** diff/intersect on *main's* new commits — `git diff --name-only $(git merge-base origin/main <branch>)..origin/main` — against the branch's changed files. **Empty intersection** → textually independent, low logical-merge risk → note in one line and proceed. **Non-empty** → the passing tests no longer cover the shared file(s); name them and recommend a re-verify on the rebased tree before landing. A flag to clear (same posture as review-status), not an auto-block.
...
- **Review-status.** If the repo has `docs/plans/README.md`, find the plan doc for this branch and check its **Reviewed** column. `No` / `Stale` (per `/wrapup` Step 2 semantics) → surface it — many of Phil's branches gate on plan sign-off before landing, and sign-off now means an approved explainer that is in sync with the plan's content. Not an automatic block, but a flag the user must clear.
- **Is every deliverable actually built?** Open the plan and list each thing it promises: every *Files to Modify* row, every numbered phase and sub-phase (1a, 1b, …), every gate. Match each one to a commit on the branch or a `file:line` in its tree. Show the list. Anything with no match is either unbuilt or was dropped on purpose, so ask which, and record the answer in the plan. Don't accept "Phase N done" as a match for its sub-parts: rolling them up is how an approved sub-phase once went unbuilt for weeks. An unmatched item doesn't block the merge, but it does block marking the plan Completed and the brief's stage landed (Phase 4).
...
- **doc/registry overlap only** — `docs/plans/README.md`, `docs/next.md`, task `.md`s, generated `.html`. **Low** risk (append-y registries resolve trivially). Note in one line; don't alarm. Plans authored on the mount cannot collide here at all, since they are not in git.
- **same-workstream layering** — the landing branch is strictly ahead of (and effectively contains) a sibling, or vice-versa. This is **one branch replacing another, not a conflict**: the sibling is likely mergeable-into / deletable-after this land, or *this* branch is superseded by a sibling that should land instead. Surface the relationship — landing the wrong one of a layered pair is a real mistake.
...
Summarize Phases 1–2 as bullets (ready to merge? reviewed? behind main? tests still current, or stale because main moved? conflicts found, and in what order do branches merge?), then **gate the write-to-main with an `AskUserQuestion`** — structured options, never inline yes/no, never silent:
...
- **Mark the plan doc done**, only if Phase 1's deliverable list came back fully matched or every gap was explicitly dropped; otherwise mark it `Partially landed` and name what's still owed. Done looks like `**Status: Completed** (<date>)` at the top of the plan doc; flip its `docs/plans/README.md` row to Completed / `Reviewed: Yes` as appropriate (per `/wrapup` Step 2 rules — don't silently promote review-status; the merge itself is the completion signal, not a review).
- **Update the brief, then prune the board.** Both live on the mount, so neither costs a review and
  both work from either machine.
  - **Brief** (`…/planning/programs/<name>.md`): set this stage's row to landed in the `Stages`
    table. If the landing settled something the user decided, append one dated line to the
    `Decided already` ledger. If the stage's row was the last one open, say so — the program may be
    finished, which is worth telling him rather than leaving the brief looking live.
  - **Board** (`…/planning/boards/<repo>.md`): **remove** the item's block. Finished work does not
    belong on a board; its status now lives in the brief's stage table. If follow-ups survive the
    landing, they become their own block with their own `next:` line rather than a note attached to
    the dead one.
  - The repo's `docs/next.md` is legacy. Don't update it as well — if it still carries the landed
    item, that is a sign it has real content that should move to the board.
- **Warn the siblings** — for any Phase 2 CODE-hotspot sibling that now needs a rebase, offer (via an `AskUserQuestion`) to add a short `⚠ Parallel-branch note` to that sibling's plan doc naming the now-landed files + that a rebase is due, so the next agent picking it up is warned. **Resolve the sibling's plan doc on the mount first** (`/mnt/su-vista-uscentral1/chaudhari_lab/phil/planning/<repo>/<stem>.md`) and write the note there. The in-repo `docs/plans` copy is frozen history: writing to it would put a new edit into git for no benefit, and the sibling's own session is reading the mount copy anyway. Only fall back to the in-repo path if the plan has no mount copy at all. Same caveats as `/vista-drift`: editing a sibling plan doc is a **write** (name the file, and use the bg-safe path if the in-repo fallback applies); **never touch sibling code — plan docs only**.

## Phase 5 — Summary

Bullet summary led by the program and stage, then `<repo> · <branch> → main`: SHA/PR landed, branch + worktree pruned (or why not), the brief's stage row updated and the board entry removed, and — if Phase 2 flagged them — which siblings now owe a rebase and in what order they should land. Close by pointing at the next branch in the recommended merge order, if any.
...
- **`/wrapup`** — owns *session*-close doc/memory/board hygiene and the branch's own **opt-in** commit gate (it doesn't commit by default — only at a milestone). `/land` reuses its Step 2 review-status semantics (one promotion path: an approved, in-sync explainer) and its Step 3 board discipline rather than redefining them. Land a branch's final commits via `/wrapup` → `/commit-review` *before* `/land`.
