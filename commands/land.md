...
- **VM gate outstanding?** If the plan's *Verification & VM handoff* section (or a `docs/vm-status/` doc) shows a smoke/run that never got its readback, the branch may be unproven. Surface it; let the user decide whether landing is appropriate.
