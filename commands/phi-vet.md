...
If the user picks **No, print it for me first**, you must reproduce the doc's content **as literal text in the body of your assistant reply** — that is the *only* channel a human can see.

> **The one invariant — the rule this whole step turns on.** The human sees **only the prose characters in your assistant message**. *Everything else is invisible to them*: tool calls and their results — bash stdout (`cat`, `git show`, `git diff`), Read-tool output, all of it — are folded by Claude Code into a collapsed "+N lines" stub the human does not expand. This holds **even in an interactive SSH terminal**. Therefore running `git show :path` / `cat` / Read surfaces the content **to you, not to them**. *Retrieving is not showing.* The ONLY act that shows a doc to the human is typing its bytes into your reply text inside a fenced code block.

**This is the single most common phi-vet failure and it recurs on essentially every run** (across agents, not just one): the agent runs `git show`, sees the content come back in the *agent-visible* tool result, writes "here's the file" / "that's the complete file," and asks for the ack — **but the human saw nothing.** That is the **self-ack trap**, and a "Yes, I read it" obtained that way is worthless. Do not let the convenience of having just run a command fool you into thinking it was displayed.

- **Retrieve into context, then RE-EMIT as prose.** You *may* load the bytes with Read or a Bash call (`git show :path/to/doc.md` for a newly-added / rewritten doc, or `git diff --cached -- path/to/doc.md` for an edit) — but treat that strictly as getting the content into *your* context. The load is not the surface. You then **transcribe those bytes into your next reply inside a fenced code block.** Never substitute "I ran `git show` above" / "see the output above" for the paste — that output is folded and invisible.
- **Hard pre-ack self-check — run it before EVERY read-confirmation `AskUserQuestion`:** *Do the doc's actual characters appear in the prose of the message I am about to send — not inside any tool-call or tool-result block?* If no → STOP and paste them first. Never fire the ack question until the answer is yes.
- **One doc per turn, hard stop-and-wait.** Surface one doc's content (as prose in your reply), ask its `AskUserQuestion`, and **wait** for an explicit "Yes, I read it" before touching the next. Do not paste several docs at once, do not pre-load the next one, and never proceed on inference or because "the file looks fine." This is what keeps the ack loop from stalling or self-acking.
- **Never use `open` / `/read-plan` / any GUI:** phi-vet runs on the headless PHI-bearing VM, with no desktop to launch a `.md` app on.

If the inlined content is ever too large to paste comfortably, that is a signal the doc is too big for a terminal appropriateness-read — say so and pause rather than silently falling back to a folded `cat`/`git show` and claiming it was shown; do not work around it with a channel the human can't see.
