...
If the user picks **No, print it for me first**, you must surface the content **as text in your own reply** — that is the *only* channel that reaches a human. **There is no terminal-print fallback**, and this holds **even in an interactive SSH terminal**: Claude Code folds/truncates tool output — bash stdout (`cat`, `git show`, `git diff`) *and* Read-tool results alike — into a collapsed "+N lines" stub, so a human sitting at the terminal sees nothing. Likewise **never use `open` / `/read-plan` / any GUI**: phi-vet runs on the headless PHI-bearing VM, with no desktop to launch a `.md` app on.

- **Re-emit, don't Read-and-claim.** Load the staged blob with a Bash call (`git show :path/to/doc.md` for a newly-added / rewritten doc, or `git diff --cached -- path/to/doc.md` for an edit on an existing one), then **paste it into your next user-facing reply inside a fenced code block.** Loading it via Read/Bash only puts the content in *Claude's* context (folded away in the TUI) — pasting it into your message text is what the human actually reads. Saying "I've read it, please confirm" without the content in your reply is the **self-ack trap** — the exact failure this step exists to prevent.
- **One doc per turn, hard stop-and-wait.** Surface one doc's content, ask its `AskUserQuestion`, and **wait** for an explicit "Yes, I read it" before touching the next. Do not paste several docs at once, do not pre-load the next one, and never proceed on inference or because "the file looks fine." This is what keeps the ack loop from stalling or self-acking.

If the inlined content is ever too large to paste comfortably, that is a signal the doc is too big for a terminal appropriateness-read — say so and pause rather than silently falling back to a folded `cat`; do not work around it with a channel the human can't see.
