...
- Then offer the natural next steps in one line: commit the plan via `/commit-review`, run `/plan-check` for fresh-agent-handoff readiness, or start implementation.
...
- **Don't fire when the user asks Claude to review the plan** ("can you check this plan?", "audit it", "any gaps?"). That's `/plan-check` territory.
