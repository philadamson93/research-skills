...
If the user picks a reviewer, invoke `/review-tests <same-plan-path> --reviewer <choice>` in the same turn — `/review-tests` owns its own flow from there. If the user picks Skip, or no offer was warranted, the default hand-off is straight to commit:
