...
- **Behavioral test design (always-on lens)**: verification steps must check *outputs against expected outputs given inputs*, not properties of generated source. Flag any plan whose verification reads `assert 'X' in rendered_<output>` — substring asserts on rendered SQL / JSON / config / formatted output test what's trivially true in source and pass coincidentally when the substring lives in unrelated fields (canonical example: `'ABD(OMEN)?' in rendered_sql` passed at vista-ct because the pattern lived in BOTH `study_include_patterns` AND `series_include_pattern`, two semantically distinct fields used by different helpers). Diagnostic question: *"What behavior does this prove?"* — if the only honest answer is "imports + render didn't crash," rewrite as render-smoke + the actual behavioral check. The four pillars of acceptable test design the plan should specify:
  1. **Behavioral over structural.** Prefer end-to-end checks (EXCEPT-DISTINCT against baseline, row-count parity, schema-set equality, materialize-and-diff) and upstream-contract asserts on registries/config (`CHEST.overlap_with == ('abd_pel',)`) over substring asserts on generated output.
  2. **Edge cases.** Plausibly-beyond-happy-path inputs raise the right error type with a regex-matched message; boundary values (zero, exact-window-day, empty collection) get exact-boundary tests.
  3. **Cross-task / cross-label logical invariants** (logically necessary given data semantics, not data-dependent). Examples: 2yr-survival labels are a subset of 3yr-survival labels for the same cohort (death is monotonic); PFS event ⇒ patient was alive at PFS time (progression precludes prior death); materialized task tables have at most one row per `(person_id, task, split)`.
  4. **Domain-knowledge sanity checks** — be creative, but **prefer relative invariants over absolute assumptions about the data distribution**. Pinning specific percentages (e.g. "5yr-survival positive rate is 40-70%") locks in the current cohort's distribution and breaks under legitimate data drift (cohort changes, OMOP migrations, treatment improvements). Use relative orderings, ratios, and self-consistency instead. Good shapes: Stage IV 5yr-mortality > Stage I 5yr-mortality (relative ordering across stages — cancer biology); 1yr-survival positive rate ≥ 5yr-survival positive rate for the same cohort (relative ordering across time horizons — alive-at-1yr is a superset of alive-at-5yr); treatment end-date ≥ start-date per row (per-row relative); median time from diagnosis to first imaging < median time from diagnosis to first treatment (relative across tasks — clinical workflow); broader-inclusion cohorts have ≥ patient counts than narrower cohorts (relative across cohort variants). Bad shapes: "positive rate falls in [40%, 70%]" (absolute distribution pin); "≥95% of patients have imaging within 5yr" (absolute threshold). **When the reviewer can't decide whether a domain invariant actually holds, raise it via `AskUserQuestion`** — domain expertise lives with the user. Don't drop uncertain domain checks; don't fabricate them either.
