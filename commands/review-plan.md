...
## Program Header
- Does the `Program:` line resolve to a brief that exists, with stage numbers that exist in that
  brief's Stages table? Does the brief's row for this stage agree with the plan's own status? Does
  anything in the plan contradict a dated entry in the brief's `Decided already` ledger?
...
- Decision point | Plan's current choice | Reusable alternative + the realistic use case it would serve | Recommendation, OR "raise to user" when realistic-use is unclear. Don't quietly assume the simpler one-off is right.

## Verification Gaps
- Missing tests, checks, or rollout validation — including verification recipes for any step run against real data or on GPU/high-throughput hardware (which queries / scripts / dataset diffs to run to confirm correctness in the real runtime, not just unit-test passage)
...
**Findings about splitting code into reusable pieces are Uncertain by default.** Whether that's worth the complexity is fundamentally a user-side product judgment about realistic future use — Claude can't decide it from code alone. Surface these findings via `AskUserQuestion` in Phase 5 (the reviewer's reusable alternative as one option, the plan's simpler one-off as another, and Claude's leaning called out) UNLESS the plan author already wrote down why the one-off is right here AND the reviewer's evidence is weak. The user has explicitly asked to err toward the reusable shape when realistic use exists, and toward raising-the-question rather than silent defaults when it doesn't.
...
**Apply the materiality filter before surfacing anything.** Sort each disagreement by what the
answer changes, and act on the tier (`claude_ops.md` → Standing rules):

- Changes the results, the cost, or what we can conclude → surface it.
- Changes only the shape of the plan or the code → decide it yourself, apply it, and record the
  choice in the plan. Do not ask.
- Reversible in under an hour → just do it.

Every question that does reach him must name **what changes in the outcome** depending on the
answer, and the default you will take if he shrugs. If you cannot name the consequence, it is not a
question — decide it. A review that surfaces six shape-of-the-code disagreements has failed at this
step even if every one of them is correct.
...
- **Reviewer's "Questions For The Author"** — these are explicit user-decision items, not findings. Append them to the plan's "Open questions" (or equivalent) section verbatim, so the user encounters them in the explainer's Open Questions panel and can resolve them there. Don't auto-resolve. If a question overlaps an Open Question already in the plan (common on a second-reviewer pass), merge rather than duplicate — keep the version with sharper phrasing.
...
> Plan revised against <Codex|fresh Claude> review (<n> findings applied; <m> open questions; concision pass <pre>→<post> lines, -X%; HTML <in sync | regenerated | stale per user>). Want the explainer regenerated so you can review it (`/explain-plan` — the only path that records a review), or run the *other* reviewer (`/review-plan <plan-path> --reviewer <other>`) for a second pass?

Don't offer to commit the plan here. For a plan authored on the shared mount — everything from
2026-09-15 on — there is no commit to offer: it never enters git, which is the whole point. For a
plan still living in a repo's `docs/plans`, it is committed only once approved, and approval means
an in-sync explainer the user signed off on (`claude_ops.md` → What gets committed). Either way,
route to `/explain-plan`, not to a commit.
