...
- **Missing domain-knowledge sanity checks.** Be creative. Where domain understanding (clinical / scientific / business) makes a sanity invariant plausible (Stage IV mortality > Stage I; label-positive-rate within plausible bounds; treatment end-date ≥ start-date; imaging dates plausible relative to diagnosis), suggest a test. **When you can't decide whether a domain invariant actually holds, mark the gap as Uncertain** so the user can confirm or refute — the user has the domain expertise. Don't drop these; don't fabricate them.
