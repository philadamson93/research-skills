...
| [`executor-mac-parity-remove-deviation-handback.md`](executor-mac-parity-remove-deviation-handback.md) | Draft | No | Retire the Mac/VM authority split entirely (full parity) and the `vm-handoff` doc mechanism it required; standalone scripts + shared bucket mount replace it for GPU/high-throughput work. Directed live in conversation rather than via `/read-plan`/`/explain-plan`. |
